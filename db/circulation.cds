...
entity Branch {
  /**
   * Assigned by the *client*: a branch code the organisation allocates, not a number the store hands
   * out. So no `Core.ComputedDefaultValue` and no entry in srv/handlers/keys.ts - a create without it
   * is a create with no key, and is refused. The reference model names this entity as the one whose
   * key the client owns; every other key here is generated.
   */
  key Id              : Integer;
      Name            : String(100) not null;
      Address         : Catalog.PostalAddress;
